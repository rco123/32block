...
//     cancelButtonText: '취소',
//     customClass: {
//       popup: 'custom-swal-dialog',    // 팝업 전체 스타일
//       container: 'custom-swal-container', // 팝업 외곽 스타일
//       title: 'custom-swal-title',     // 제목에 대한 스타일
//       content: 'custom-swal-content', // 본문에 대한 스타일
//       input: 'custom-swal-input',     // 입력 필드에 대한 스타일
//       confirmButton: 'custom-swal-confirm-button', // 확인 버튼 스타일
//       cancelButton: 'custom-swal-cancel-button'    // 취소 버튼 스타일
//     }
//   });
...
CustomDialog.show = function(title, message, options) {
  const swalOptions = {
    ...
    showConfirmButton: options.showOkay || false,
    confirmButtonText: '확인',
    cancelButtonText: '취소',
    input: options.showInput ? 'text' : null,
    inputValue: options.showInput ? '' : null,
    preConfirm: (inputValue) => {
      if (options.onOkay) {
        options.onOkay(inputValue);
      }
    },
    width: 200, // 다이얼로그 너비를 화면의 30%로 설정
  };
