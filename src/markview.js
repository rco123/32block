...
exports.fxMarkView = () => {
    ...
    // 컨테이너의 ID 설정
    const containerId = "image-mark-view-container";
    // 이미 생성된 컨테이너가 있는지 확인
    ...
    // 컨테이너의 ID 설정
    const checkId = "image-lane-view-container";
    // 이미 생성된 컨테이너가 있는지 확인
    ...
    const imageMarkViewContainer = document.createElement('div');
    imageMarkViewContainer.id = containerId; // 고유 ID 설정
    imageMarkViewContainer.className = "image-Mark-View-Container"

    // 이미지와 캔버스를 담을 컨테이너 생성
    const buttonContainer = document.createElement('div');
    buttonContainer.className = "button-Container"

    const buttonMark0 = document.createElement('button');
    buttonMark0.className = "btn btn-success btn-sm lane-button"
    buttonMark0.innerText = "mark0"
    const buttonMark1 = document.createElement('button');
    buttonMark1.className = "btn btn-success btn-sm lane-button"
    buttonMark1.innerText = "mark1"
    const buttonMark2 = document.createElement('button');
    buttonMark2.className = "btn btn-success btn-sm lane-button"
    buttonMark2.innerText = "mark2"

    // 클릭 이벤트 핸들러 등록
    buttonMark0.addEventListener('click', () => {
        console.log("Mark0 버튼이 클릭되었습니다.");
        const localDir = 'C:\\Users\\sgkim\\Desktop\\aiimage\\mark\\mark0';
        const images = loadAllImages(localDir); // 이미지 목록 가져오기
        createCards(images); // 카드 생성
    });
    // 클릭 이벤트 핸들러 등록
    buttonMark1.addEventListener('click', () => {
        console.log("Mark1 버튼이 클릭되었습니다.");
        const localDir = 'C:\\Users\\sgkim\\Desktop\\aiimage\\mark\\mark1';
        const images = loadAllImages(localDir); // 이미지 목록 가져오기
        createCards(images); // 카드 생성
    });
    // 클릭 이벤트 핸들러 등록
    buttonMark2.addEventListener('click', () => {
        console.log("Mark2 버튼이 클릭되었습니다.");
        const localDir = 'C:\\Users\\sgkim\\Desktop\\aiimage\\mark\\mark2';
        const images = loadAllImages(localDir); // 이미지 목록 가져오기
        createCards(images); // 카드 생성
    });

    buttonContainer.appendChild(buttonMark0)
    buttonContainer.appendChild(buttonMark1)
    buttonContainer.appendChild(buttonMark2)

    const cardContainer = document.createElement('div')
    cardContainer.className = 'card-Container'

    imageMarkViewContainer.appendChild(buttonContainer)
    imageMarkViewContainer.appendChild(cardContainer)

    const blocklyArea = document.getElementById('blocklyArea');
    blocklyArea.appendChild(imageMarkViewContainer)
